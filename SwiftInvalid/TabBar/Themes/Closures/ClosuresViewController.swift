...
class ClosuresViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let originalImage = UIImage(named: "Замыкания")
        ...
        LableInfo.attributedText = imageString
            ...
            button.setTitle("Замыкающие выражения", for: .normal)
            button.addTarget(self, action: #selector(buttonTapped), for: .touchUpInside)
            button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 17) // Установка жирного шрифта для заголовка кнопки
            button.frame = CGRect(x: scrollView.contentSize.width - 280, y: scrollView.contentSize.height, width: 300, height: -100)
            scrollView.addSubview(button)
        }
        
        // действие при нажатии на кнопку
        @objc func buttonTapped() {
            let NextVC = UIStoryboard(name: "TabBar", bundle: nil).instantiateViewController(withIdentifier: "ClosingExpressionsViewController") as! ClosingExpressionsViewController
