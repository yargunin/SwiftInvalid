...
class DeinitializationViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let originalImage = UIImage(named: "Деинициализация")
        ...
        LableInfo.attributedText = imageString
            ...
            button.setTitle("Опциональная последовательность", for: .normal)
            button.addTarget(self, action: #selector(buttonTapped), for: .touchUpInside)
            button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 17) // Установка жирного шрифта для заголовка кнопки
            button.frame = CGRect(x: scrollView.contentSize.width - 330, y: scrollView.contentSize.height, width: 300, height: -100)
            scrollView.addSubview(button)
        }
        
        // действие при нажатии на кнопку
        @objc func buttonTapped() {
            let NextVC = UIStoryboard(name: "TabBar", bundle: nil).instantiateViewController(withIdentifier: "OptionalSequenceViewController") as! OptionalSequenceViewController
