...
class LectureViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let originalImage = UIImage(named: "Константы и переменные")
        ...
        button.setTitle("Аннотация типов", for: .normal)
        button.addTarget(self, action: #selector(buttonTapped), for: .touchUpInside)
        button.titleLabel?.font = UIFont.boldSystemFont(ofSize: 17) // Установка жирного шрифта для заголовка кнопки
        button.frame = CGRect(x: scrollView.contentSize.width - 160, y: scrollView.contentSize.height, width: 150, height: -100)
